...
class HttpClient extends EventEmitter {
    ...
    constructor(config) {
        ...
        /*
         * Keep transport ownership scoped to this client. Besides isolating
         * connection statistics, this ensures shutdown never destroys
         * sockets shared with unrelated application code via globalAgent.
         */
    ...
    _executeOnceWithAuth(op, req, auth, endSend, callback) {
        ...
        // Finding 16: Must check if certificate includes port
        // if (Object.hasOwn(this._url,'port') &&
        //      Object.hasOwn(this._config._url,'port') &&
        //      this._url.port != this._config._url.port) {
        //     throw new Error('Transport host provided port: ' +
        //          this._config._url.port + ' does not match the signed port: ' + this._url.port);
        // }
        ...
        const httpReq = this._httpMod.request(httpOpt, (res) => {
            this._captureStatsConnectionCount(req);
        ...
        httpReq.on('socket', () => {
            /* Let Agent register the active socket before sampling it. */
            process.nextTick(() => this._captureStatsConnectionCount(req));
        });
    ...
    _getConnectionCount() {
        let count = 0;
        if (this._agent == null || this._agent.sockets == null) {
            return count;
        }
        for (const sockets of Object.values(this._agent.sockets)) {
            count += sockets.length;
        }
        return count;
    }

    _captureStatsConnectionCount(req) {
        try {
            if (this._shouldObserveStats()) {
                req._statsConnectionCount = this._getConnectionCount();
            }
        } catch(err) {
            /* Stats collection must never change SDK request behavior. */
    ...
    _shouldObserveStats() {
        return this._stats._isCollecting();
    }
    ...
    _observeStatsError(op, req, err) {
        ...
        this._observeStats('_observeError', op.name,
            req._statsConnectionCount, req._statsRetryCount,
    ...
    async execute(op, req) {
        ...
        op.validate(req);
        req._op = op;
        if (req._statsObserveQuery) {
            /*
             * Count the logical query once, after validation succeeds. Internal
             * QueryOp fetches are still counted as HTTP requests separately.
             */
            delete req._statsObserveQuery;
            this._observeStats('_observeQuery', req);
        }
        ...
        try {
            for(;;) {
                ...
                try {
                    ...
                    timeout = err.errorCode ===
                        ...
                        Math.max(req.opt.securityInfoTimeout,
                            req.opt.timeout) : req.opt.timeout;
            ...
            /*
             * Observe every terminal execution failure exactly once, including
             * exceptions from retry policy, protocol handling and rate limiting.
             */
    ...
    shutdown() {
        /*
         * Flush the final stats snapshot and stop the interval timer before
         * closing HTTP resources.
         */
