...
class HttpClient extends EventEmitter {
    ...
    _handleResponse(op, req, res, buf, callback) {
        try {
            if (res.statusCode == HttpConstants.HTTP_OK) {
                ...
                var cookie = this._getSessionCookie(res.headers);
                if (cookie != null) {
    ...
    _getSessionCookie(headers) {
        return headers[Object.keys(headers).find(key =>
          key.toLowerCase() === HttpConstants.SET_COOKIE)];
    }
