...
const atomicsEqual = require('./utils').atomicsEqual;
...
${step.limitStep.displayName} for SFW`);            
    ...
    _groupInputRow() {
        ...
        for(i = 0; i < this._gbRow.length; i++) {
            if (!atomicsEqual(this, this._colIters[i].result,
                this._gbRow[i])) {
                break;
            }
        }
...
SFWIterator for selectStar');
        ...
        for(let i = 0; i < this._colIters.length; i++) {
            ...
            res[this._step.colNames[i]] = hasRes ? it.result : null;
            it.reset();
        }
        this.result = res;
        return true;
