...
${bindCnt} does not match expected ${this._extVars.length}`);
    ...
    async execute(req) {
        ...
        if (req._statsObserveQuery) {
            /*
             * Advanced query execution may reuse the same request for internal
             * fetches. Record the logical query once and then remove the marker.
             */
            delete req._statsObserveQuery;
            this._client._observeQueryStats(req);
        }
