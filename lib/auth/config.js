...
class AuthConfig {
    ...
    static _initServiceType(cfg) {
        ...
        const ownsAuth = cfg.hasOwnProperty('auth');
        const isIAM = ownsAuth && cfg.auth.hasOwnProperty('iam');
        const isKVStore = ownsAuth && cfg.auth.hasOwnProperty('kvstore');
        if (isIAM && isKVStore) {
...
type');
        ...
        if (isIAM) {
            ...
            cfg.serviceType = ServiceType.CLOUDSIM;
        }
    }
