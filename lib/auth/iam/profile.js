...
class IAMProfileProvider {
    ...
    constructor(creds, cfg) {
        ...
        if (!Utils.isValidOcid(creds.userId)) {
            throw new NoSQLArgumentError(
                `auth.iam.userId is not a valid OCID: ${creds.userId}`, cfg);
        }
        this._userId = creds.userId;

        if (!('fingerprint' in creds)) {
            throw new NoSQLArgumentError('Missing auth.iam.fingerprint', cfg);
        }
        if (typeof creds.fingerprint !== 'string' ||
            !creds.fingerprint.length) {
            throw new NoSQLArgumentError(`Invalid auth.iam.fingerprint: \
...
auth.iam.privateKeyFile and auth.iam.privateKey', cfg);
            ...
            if (!('privateKey' in creds)) {
                throw new NoSQLArgumentError('Must set one of \
auth.iam.privateKeyFile or auth.iam.privateKey, both are missing', cfg);
            }
...
class UserProfileProvider {
    ...
    constructor(cfg) {
        assert(cfg.auth.iam != null && 'credentialsProvider' in cfg.auth.iam);
