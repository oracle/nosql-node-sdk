...
function _hasUserIdenProps(opt) {
    return opt.privateKey != null || opt.privateKeyFile != null ||
        opt.configFile != null || opt.profileName != null ||
        opt.credentialsProvider != null;
}
...
more than ${MAX_ENTRY_LIFE_TIME} seconds`, cfg);
        ...
        if (opt.useResourcePrincipal) {
            if (opt.useInstancePrincipal || _hasUserIdenProps(opt)) {
                throw new NoSQLArgumentError('Cannot specify both \
useResourcePrincipal and useInstancePrincipal or user identity properties in \
auth.iam', cfg);
            }
            this._provider = new ResourcePrincipalProvider(cfg);
        } else if (opt.useInstancePrincipal) {
            if (_hasUserIdenProps(opt)) {
                throw new NoSQLArgumentError('Cannot specify both \
useInstancePrincipal and user identity properties in auth.iam', cfg);
            }
