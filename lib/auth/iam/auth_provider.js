...
const OKEWorkloadIdentityProvider = require('./oke_workload');
...
const OKE_WORKLOAD_PROP = 'useOKEWorkloadIdentity';

const OKE_EXTRA_PROPS = [ 'serviceAccountToken',
    'serviceAccountTokenFile', 'serviceAccountTokenProvider' ];
...
more than ${MAX_ENTRY_LIFE_TIME} seconds`, cfg);
        ...
        if (opt.useResourcePrincipal) {
            _chkExclProps(cfg, opt, RES_PRINCIPAL_PROP, USER_IDEN_PROPS,
                [ INST_PRINCIPAL_PROP, OKE_WORKLOAD_PROP, SESS_TOKEN_PROP ]);
...
without ${RES_PRINCIPAL_PROP}`, cfg);
            ...
            _chkExclProps(cfg, opt, INST_PRINCIPAL_PROP, USER_IDEN_PROPS,
                [ OKE_WORKLOAD_PROP, SESS_TOKEN_PROP ]);
...
without ${INST_PRINCIPAL_PROP}`, cfg);
        } else if (opt.useOKEWorkloadIdentity) {
            _chkExclProps(cfg, opt, OKE_WORKLOAD_PROP, USER_IDEN_PROPS,
                [ SESS_TOKEN_PROP ]);
            this._provider = new OKEWorkloadIdentityProvider(opt, cfg);
        } else if ((prop = _findProps(opt, OKE_EXTRA_PROPS)) != null) {
            throw new NoSQLArgumentError(`Cannot specify property ${prop} \
without ${OKE_WORKLOAD_PROP}`, cfg);
...
${HttpConstants.CONTENT_SHA256}: ${reqSigning.digest}`;
    ...
    getRegion() {
        if (this._provider.getRegion != null) {
            return Promise.resolve(this._provider.getRegion());
        }
        if (this._provider.getRegionFromIMDS != null) {
            return this._provider.getRegionFromIMDS();
        }
    }
    ...
    static withOKEWorkloadIdentity(serviceAccountToken) {
        return new IAMAuthorizationProvider(
            typeof serviceAccountToken === 'string' ? {
                useOKEWorkloadIdentity: true,
                serviceAccountToken
            } : {
                useOKEWorkloadIdentity: true,
                serviceAccountTokenProvider: serviceAccountToken
            });
    }

    static withOKEWorkloadIdentityAndTokenFile(serviceAccountTokenFile) {
        return new IAMAuthorizationProvider({
            useOKEWorkloadIdentity: true,
