 ...
 * 
 * If you set {@link KVStoreCredentials}#autoRenew property to true, after
 * initial login is done, the driver will renew authentication token each time
 * it reaches half of its lifetime (half-point between acquisition and
 * expiration).  Renew request only requires existing token and does not
 * require user credentials.  If renew request fails, the token will
 * eventually expire and the driver will perform another login (errors due
 * to token expiration are automatically retried by the driver).
 * 
 ...
 * requests
 * @property {boolean} [autoRenew] If set to true, the driver will attempt to
 * renew the authentication token half way before its expiration time
 */
