...
/*
 * Maps Node.js internal operation class names to the common request names used
 * in SDK statistics. Keeping these names consistent across SDKs allows the same
 * dashboards, log parsers, and monitoring rules to process their stats output.
 */
...
/*
 * Exact latency percentile collector. Java stores latency samples for the
 * interval and sorts them when the snapshot is generated.
 */
class Percentile {
...
/*
 * Per-request-type metrics bucket, equivalent to Java Stats.ReqStats.
 * It tracks counts, errors, retry totals, latency, request size and result
 * size for one request name such as Get, Put, Query or Table.
 */
class ReqStats {
...
/*
 * Tracks concurrent connection samples for the interval. The JSON shape
 * matches Java: only min, max and avg are emitted.
 */
...
/*
 * Query helper functions read query text, prepared statement and plan details
 * from the Node request/result shapes used by simple and advanced queries.
 */
...
/*
 * Stats for one SQL query text under ALL profile. The top-level count is the
 * logical query count, while reqStats records the HTTP QueryOp executions.
 */
...
/*
 * ALL-profile query statistics. This mirrors Java ExtraQueryStats by grouping
 * stats per SQL text and keeping query metadata such as plan/simple/writes.
 */
class ExtraQueryStats {
    ...
    observeQueryRequest(queryReq, queryRes, error, retryCount, retryDelayMs,
...
/*
 * Aggregates all interval statistics for one client. StatsControl owns this
 * object and asks it to observe requests, query metadata and connections.
 */
class Stats {
    ...
    observe(opName, error, connections, retryCount, retryDelayMs,
        ...
        if (opName === 'QueryOp' && profileAtLeast(this.profile, 'ALL')) {
             ...
             * snapshot is emitted.
             */
            this.extraQueryStats.observeQueryRequest(queryReq, queryRes, error,
    ...
    observeQuery(queryReq, queryRes) {
        ...
        this.extraQueryStats.observeQuery(queryReq, queryRes);
    }
