...
const SyncPolicy = require('../constants').SyncPolicy;
const ReplicaAckPolicy = require('../constants').ReplicaAckPolicy;
...
specified for one end of field range');
    ...
    static serialize(pm, dw, req, serialVersion) {
        try {
            pm.serializer(this.name).serialize(dw, req, serialVersion);
            this.chkRequestSizeLimit(dw, req);
        } catch(err) {
            throw new NoSQLArgumentError(
                'Error processing request, see the cause', req, err);
        }
    }

    //req argument is only needed for PrepareSerializer.deserialize()
    //and QuerySerializer.deserialize()
    static deserialize(pm, dr, req, serialVersion) {
        try {
            return pm.serializer(this.name).deserialize(dr, req, serialVersion);
        } catch(err) {
