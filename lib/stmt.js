...
class BatchCursor {
    ...
    _next(callback) {
        if (!this.hasNext()) {
            return process.nextTick(() => callback(new NoSQLArgumentError(
                'This cursor has no more records')));
        }
        return this._client._query(this._stmt, this._opt, (err, res) => {
            if (err) {
                return callback(err);
            }
            this._first = false;
     ...
     * Can be used with callback or Promise like the methods in
     * {@link NoSQLClient} class. If this method is called after
     * {@link BatchCursor#hasNext} returned false, it will result in error.
     * @async
     * @returns {Promise} Promise of {@link QueryResult}
     */
    next() {
        return promisified(this, this._next);
    }
}
