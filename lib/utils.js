...
function _readProfile(configFile, profileName, callback) {
    ...
    ri.on('line', line => {
        ...
        if (profile == null || finished) {
            return;
        }
        let i = line.indexOf('=');
        if (i == -1) {
            return callback(new NoSQLArgumentError(`Invalid line in config \
file ${configFile} (no key-value): ${line}`));
        }
        const key = line.substring(0, i).trim();
        if (!key.length) {
            return callback(new NoSQLArgumentError(`Invalid line in config \
file ${configFile} (empty key): ${line}`));
        }
        profile[key] = line.substring(i + 1).trim();
    });
    ri.on('close', () => { callback(null, profile); });
