...
class NoSQLClient extends NoSQLClientImpl {
     ...
     * filter and parse. The snapshot is logged when
     * <code>statsEnableLog</code> is <code>true</code> and delivered to
     * <code>statsHandler</code> when a handler is configured. After the
     ...
     * <code>statsHandler</code> that writes the serialized snapshot to that
     * file and set <code>statsEnableLog</code> to <code>false</code> to avoid
     ...
     * }
     *
     * @example <caption>Writing statistics snapshots to a file</caption>
     * const fs = require('fs');
     * const statsFile = fs.createWriteStream('./client-stats.log', {
     *     flags: 'a'
     * });
     *
     * const client = new NoSQLClient({
     ...
     * });
     *
     * @returns {StatsControl} Statistics control object
     * @see {@link StatsControl}
     * @see {@link StatsProfile}
     * @see {@link StatsSnapshot}
     */
    getStatsControl() {
        return super.getStatsControl();
    }
    ...
    setTableTags(tableName, definedTags, freeFormTags, opt) {
        ...
        return opt.complete ?
            this._withCompletion(ops.TableTagsOp, req) :
            this._execute(ops.TableTagsOp, req);
    }
 ...
 * table reaching a certain state.  Note that this event may be emitted multiple
