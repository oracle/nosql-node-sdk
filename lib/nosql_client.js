 ...
 * Some parameters, such as the service endpoint, are required. Other
 * parameters are optional and need not be specified in the {@link
 * Config}. Default values are used for optional parameters.
 * <p>
...
class NoSQLClient extends NoSQLClientImpl {
     ...
     * @param {string|Config} config Configuration for NoSQL client. May be
     * either a string indicating the file path to a configuration
     ...
     * {@link NoSQLClient#forCompletion} (or alternatively
     * {@link NoSQLClient#forTableState}) to asynchronously wait for the
     ...
     * @see {@link NoSQLClient#forTableState}
     ...
     * long-running operations, using {@link NoSQLClient#forCompletion},
     * {@link NoSQLClient#forTableState} and options <em>opt.complete</em> and
     * <em>opt.delay</em> apply to this API.  See
     ...
     * achieved by polling the table at specified intervals. The state of
     * {@link TableState.DROPPED} is treated specially in that it will be
     * returned as success, even if the table does not exist. Other states
     * will result in error if the table is not found.
     * <p>
     * This API is used to ensure that the table is ready for data
     * operations after it has been created or altered. It waits until
     ...
     * can be used.
     * <p>
     * The result of this operation, if successful, is a {@link
     * TableResult} that shows the table state from the last poll. The
     ...
     * found.
     * <p>
     * Another way to asynchronously wait for completion of operation
     * initiated by {@link NoSQLClient#tableDDL} is to use
     * {@link NoSQLClient#forCompletion} or pass <em>opt.complete</em>
     * to {@link NoSQLClient#tableDDL}
     * @example // Using forTableState
     * try {
     *     let tableRes = await client.tableDDL('CREATE TABLE.....');
     *     await client.forTableState(tableRes, TableState.ACTIVE);
     *     let putRes = client.put(//.......
     * } catch(err) {
     *     // May be caused by client.forTableState if long running DDL
     *     // operation was unsuccessful, see "table" parameter.
     * }
     * @async
     * @param {string|TableResult} table Either the table name or a
     * {@link TableResult} object that was returned from a call to
     * {@link NoSQLClient#tableDDL}. If the latter,
     * error information for the DDL operation will be retrieved, so
     * if the original call failed, this follow-on call will also fail with
     ...
     * @see {@link NoSQLClient#forCompletion}
     */
    forTableState(table, tableState, opt) {
        return this._forTableState(table, tableState, opt);
    }
     ...
     * method is equivalent to calling {@link NoSQLClient#forTableState} and
     * asynchronously waiting for state {@link TableState.ACTIVE} for all
 ...
 * also while table is polled waiting for its state change by
 * {@link NoSQLClient#forTableState}.  Can be used to perform actions on a
