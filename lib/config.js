...
than table poll delay', cfg);
        ...
        try {
            cfg.statsProfile = Stats.normalizeProfile(cfg.statsProfile);
        } catch(err) {
            throw new NoSQLArgumentError(
                `Invalid statsProfile value: ${cfg.statsProfile}`, cfg, err);
        }
        try {
            cfg.statsLatencyPercentileMode =
                Stats.normalizeLatencyPercentileMode(
                    cfg.statsLatencyPercentileMode);
        } catch(err) {
            throw new NoSQLArgumentError(
                `Invalid statsLatencyPercentileMode value: \
${cfg.statsLatencyPercentileMode}`, cfg, err);
        }
        if (!isPosInt32(cfg.statsInterval)) {
            throw new NoSQLArgumentError(
                `Invalid statsInterval value: ${cfg.statsInterval}`, cfg);
        }
        if (typeof cfg.statsPrettyPrint !== 'boolean') {
            throw new NoSQLArgumentError(
                `Invalid statsPrettyPrint value: ${cfg.statsPrettyPrint}`,
                cfg);
        }
        if (typeof cfg.statsEnableLog !== 'boolean') {
            throw new NoSQLArgumentError(
                `Invalid statsEnableLog value: ${cfg.statsEnableLog}`, cfg);
        }
        if (cfg.statsHandler != null &&
            !StatsControl.isStatsHandler(cfg.statsHandler)) {
            throw new NoSQLArgumentError('Invalid statsHandler value', cfg);
        }
...
Config.defaults = Object.freeze({
    ...
    maxMemoryMB: 1024,
    statsProfile: Stats.Profile.NONE,
    statsInterval: StatsControl.DEFAULT_INTERVAL,
    statsPrettyPrint: false,
    statsEnableLog: false,
    statsHandler: null,
    statsLatencyPercentileMode: Stats.LatencyPercentileMode.EXACT,
    retry: Object.freeze({
        maxRetries: 10,
        baseDelay: 200,
