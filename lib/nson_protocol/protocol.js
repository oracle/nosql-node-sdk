...
class Protocol {
    ...
    static _writeMapEntries(nw, ent, opt) {
        if (opt._writeSortedMaps) {
            ...
            ent = BinaryProtocol._sortMapEntries(ent);
        }
...
field value: ${key}, must be a string`);
    ...
    //Assuming in V4 writeMany with multiple tables is supported so the
    //workaround done in BinaryProtocl._createError is not needed.  Make
    //mapError() use _createError above instead.
    static mapError(rc, msg, req) {
        return BinaryProtocol.mapError.call(this, rc, msg, req);
    }
