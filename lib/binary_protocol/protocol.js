...
class Protocol {
    ...
    static _createError(errCode, msg, req) {
        ...
        //[table1, table2, table3]
        if (errCode === ErrorCode.TABLE_NOT_FOUND &&
            //Using string comparison for op's name to avoid introducing
            //another dependency on ops module.
            req._op.name === 'WriteMultipleOp' && req.tableName == null &&
            msg && msg.includes(',') && !msg.includes('[')) {
            errCode = ErrorCode.OPERATION_NOT_SUPPORTED;
            msg = 'WriteMany operation with multiple tables is not supported \
by the version of the connected server';
        }
