...
class Protocol {
    ...
    static writeOpCode(dw, opCode) {
        dw.writeInt16BE(SERIAL_VERSION);
        dw.writeByte(opCode);
    }
    ...
    static serializeWriteRequest(dw, req) {
        this.serializeRequest(dw, req);
        dw.writeString(req.tableName);
    ...
    static readRecord(dr, opt) {
        //For now we already represent map type as object
        return this.readFieldValue(dr, opt);
    }
    ...
    static deserializeWriteResponse(dr, opt, res) {
    ...
    static deserializeWriteResponseWithId(dr, opt, res) {
        this.deserializeWriteResponse(dr, opt, res);
    ...
    static deserializeTableResult(dr, opt) {
        const res = {};
        ...
        if (hasStaticState) {
            ...
            if (opt.serviceType !== ServiceType.KVSTORE) {
                res.tableLimits = { readUnits, writeUnits, storageGB };
            }
