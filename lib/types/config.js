 ...
 *
 * @typedef {object} StatsLatency
 * @property {number} min Minimum latency
 * @property {number} max Maximum latency
 * @property {number} avg Average latency
 * @property {number} [95th] 95th percentile for MORE and ALL profiles
 * @property {number} [99th] 99th percentile for MORE and ALL profiles
 */

/**
 * Retry totals collected for a request or query.
 *
 * @typedef {object} StatsRetry
 * @property {number} count Retry count
 * @property {number} delayMs Total retry delay in milliseconds
 * @property {number} authCount Authentication retry count
 * @property {number} throttleCount Throttling retry count
 */
 ...
 * @property {string} name Request operation name
 * @property {number} httpRequestCount HTTP request count
 * @property {number} errors Error count
 * @property {StatsRetry} retry Retry totals
 * @property {number} rateLimitDelayMs Total rate-limit delay
 * @property {StatsLatency} [httpRequestLatencyMs] Request latency values
 * @property {StatsValue} [requestSize] Request size values
 * @property {StatsValue} [resultSize] Result size values
 */
 ...
 * @property {string} query SQL statement
 * @property {number} count Logical query count
 * @property {number} unprepared Unprepared query count
 * @property {boolean} simple Whether this is a simple query
 * @property {boolean} doesWrites Whether this query performs writes
 * @property {string} [plan] Query plan, when available
 * @property {number} httpRequestCount HTTP request count
 * @property {number} errors Error count
 * @property {StatsRetry} retry Retry totals
 * @property {number} rateLimitDelayMs Total rate-limit delay
 ...
 * @property {string} startTime Interval start time
 * @property {string} endTime Interval end time
 * @property {string} clientId Client identifier
 * @property {StatsValue} [connections] Connection statistics
 * @property {StatsQuery[]} [queries] Detailed query statistics
 * @property {StatsRequest[]} requests Request operation statistics
 */

/**
 * Handler function called with a statistics snapshot at the end of each stats
 * interval.
 *
 * @callback StatsHandlerFunction
 * @param {StatsSnapshot} stats Statistics snapshot
 */

/**
 * Handler object called with a statistics snapshot at the end of each stats
 * interval.
 *
 * @typedef {object} StatsHandlerObject
 * @property {function(StatsSnapshot):void} accept Handles a statistics
 * snapshot
 */
 ...
 *
 * @typedef {StatsHandlerFunction|StatsHandlerObject} StatsHandler
 */
...
/**
  ...
  * @property {string} [statsProfile=NONE] Statistics collection profile.
  * Valid values are NONE, REGULAR, MORE, and ALL.  MORE includes 95th and
  * 99th percentile latency in addition to REGULAR request statistics.
  * @property {number} [statsInterval=600] Statistics collection interval in
  * seconds. Interval snapshots are aligned to the top of the hour.
  * @property {boolean} [statsPrettyPrint=false] Whether interval statistics
  * log output should be pretty-printed.
  * @property {boolean} [statsEnableLog=true] Whether interval statistics
  * should be logged.
  * @property {StatsHandler} [statsHandler] Handler called with a statistics
  * snapshot at the end of each stats interval.
